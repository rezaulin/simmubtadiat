...
func GetAbsensiPengajarKuartal(ctx context.Context, tingkatanID int, kelasID int, tahunAjaran string) ([]AbsensiPengajarKuartal, error) {
	...
	query := `
		SELECT DISTINCT p.id, p.nama, t.nama, k.nama,
		...
		JOIN kelas k ON k.id = asg.kelas_id
		JOIN tingkatan t ON t.id = asg.tingkatan_id
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.PengajarID, &a.PengajarNama, &a.TingkatanNama, &a.KelasNama, &a.Kuartal1, &a.Kuartal23, &a.Kuartal4, &a.ID); err != nil {
			return nil, err
		}
